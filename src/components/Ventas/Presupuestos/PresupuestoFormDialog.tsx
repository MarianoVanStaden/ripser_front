...
import { clienteApi, leadApi, productApi } from "../../../api/services";
...
import type { DetalleForm, FormData, TipoDescuento } from './types';
import RevestimientoDialog from './dialogs/RevestimientoDialog';
import EnvioDialog from './dialogs/EnvioDialog';
import { initialDetalle, initialFormData } from './constants';
import ConfirmPresupuestoDialog from './dialogs/ConfirmPresupuestoDialog';

interface PresupuestoFormDialogProps {
  open: boolean;
  /** null = alta; con valor = edición (o vista si readOnly). */
  presupuesto: DocumentoComercial | null;
  readOnly: boolean;
  /** Deep-link desde el detalle de lead: preselecciona el lead en modo alta. */
  initialLeadId?: string | null;
  usuarioOptions: Usuario[];
  isAdmin: boolean;
  canReassignVendedor: boolean;
  /** Resuelve la opción de financiamiento seleccionada (mapa que vive en la página). */
  getSelectedFinancingOption: (presupuesto: DocumentoComercial) => OpcionFinanciamientoDTO | undefined;
  onClose: () => void;
  /** Se dispara tras persistir OK; el padre invalida la query y muestra snackbar/success. */
  onSaved: (saved: DocumentoComercial, isNew: boolean) => void;
}

/**
 * Formulario de alta/edición/vista de presupuesto (Etapa 6.4: extraído de
 * PresupuestosPage, que quedó como orquestador de la lista). Es dueño de todo
 * el estado del form: datos, detalles, catálogos (productos/recetas, cargados
 * al primer open), typeahead de leads, confirmaciones y pre-check de deuda.
 */
export default function PresupuestoFormDialog({
  open,
  presupuesto,
  readOnly,
  initialLeadId,
  ...
  isAdmin,
  canReassignVendedor,
  getSelectedFinancingOption,
  onClose,
  onSaved,
}: PresupuestoFormDialogProps) {
  const { user } = useAuth();

  // Aliases para conservar los identificadores del JSX movido desde la página.
  const dialogOpen = open;
  const editingPresupuesto = presupuesto;
  ...
  useEffect(() => {
    ...
    if (presupuesto) {
      ...
      if (initialLeadId) {
        ...
        const leadIdNum = Number(initialLeadId);
        if (Number.isFinite(leadIdNum) && leadIdNum > 0) {
          // Sembrar el input del typeahead con el nombre para que el
          // Autocomplete resuelva y muestre el lead preseleccionado.
    ...
    // Poblar solo al (re)abrir: presupuesto/initialLeadId quedan fijados en ese momento.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [open]);

  /** Reset completo + cierre (única salida del dialog). */
  const cerrarYResetear = useCallback(() => {
    setFormData({ ...initialFormData, usuarioId: isAdmin ? '' : (user?.id ?? 0).toString() });
    setSelectedCliente(null);
    setDestinatarioMode('CLIENTE');
    setDetalles([]);
  ...
  }, [hasUnsavedChanges, readOnly, cerrarYResetear]);
  ...
  const handleSavePresupuesto = useCallback(async () => {
    ...
    try {
      ...
      const esAlta = !editingPresupuesto;
      cerrarYResetear();
      onSaved(savedPresupuesto, esAlta);
    } catch (err: unknown) {
  ...
  }, [user, isAdmin, formData, detalles, editingPresupuesto, confirmDialogAction, canReassignVendedor, canalVenta, gestionanteNombre, gestionanteTelefono, cerrarYResetear, onSaved, checkClienteDeuda]);
  ...
  return (
    <>
      <LoadingOverlay open={formLoading} message="Guardando presupuesto..." />
      <Dialog
        open={dialogOpen}
