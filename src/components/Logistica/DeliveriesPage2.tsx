    ...
    <Box sx={{ pb: isMobile ? 10 : 3, minHeight: '100vh' }}>
      ...
      <Box sx={{ p: { xs: 2, sm: 2, md: 3 }, pt: { xs: 2 } }}>
        ...
        >
          ...
          >
            {[
              { label: 'Pendientes', count: pendingDeliveries.length, color: 'warning', icon: <ScheduleIcon /> },
              { label: 'Entregadas', count: deliveredCount, color: 'success', icon: <CheckIcon /> },
              { label: 'Sin Asignar', count: unassignedDeliveries.length, color: 'error', icon: <CancelIcon /> },
            ].map((stat) => (
              <Card
                key={stat.label}
                sx={{
                ...
                onClick={() => {
                  if (stat.label === 'Pendientes') setStatusFilter('PENDIENTE');
                  else if (stat.label === 'Entregadas') setStatusFilter('ENTREGADA');
                  else setStatusFilter('all');
                }}
              >
                <CardContent sx={{ p: 1.5, '&:last-child': { pb: 1.5 } }}>
                  <Box display="flex" alignItems="center" gap={1}>
                    <Badge badgeContent={stat.count} color={stat.color as any} max={99}>
