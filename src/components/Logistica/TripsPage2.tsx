    ...
    <Box sx={{ pb: isMobile ? 10 : 3, minHeight: '100vh' }}>
      ...
      <Box sx={{ p: { xs: 2, sm: 2, md: 3 }, pt: { xs: 2 } }}>
        ...
        >
          ...
          >
            {[
              { label: 'Planificados', count: trips.filter(t => t.estado === 'PLANIFICADO').length, color: 'info', icon: <ScheduleIcon /> },
              { label: 'En Curso', count: trips.filter(t => t.estado === 'EN_CURSO').length, color: 'warning', icon: <RouteIcon /> },
              { label: 'Completados', count: trips.filter(t => t.estado === 'COMPLETADO').length, color: 'success', icon: <CheckIcon /> },
              { label: 'Vehículos', count: vehicles.length, color: 'primary', icon: <TruckIcon /> },
            ].map((stat) => (
              <Card
                key={stat.label}
                sx={{
