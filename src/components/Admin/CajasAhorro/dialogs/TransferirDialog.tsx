...
import { extractError, formatUSD, todayString } from '../utils';
...
const TransferirDialog: React.FC<Props> = ({
  open,
  onClose,
  onSuccess,
  ...
  const [cajas, setCajas] = useState<CajaAhorroDolares[]>(cajasInjected ?? []);
  ...
  const transferirMutation = useMutation({
    mutationFn: () => cajasAhorroApi.transferir({
      cajaOrigenId: Number(origenId),
      cajaDestinoId: Number(destinoId),
      montoUsd: round2(montoNum),
    ...
    <Dialog open={open} onClose={() => !saving && onClose()} maxWidth="md" fullWidth>
      <DialogTitle>
        <Box display="flex" alignItems="center" gap={1}>
          ...
          <span>Transferir entre cajas USD</span>
        </Box>
      </DialogTitle>
              ...
              >
                ...
                <Stack direction={{ xs: 'column', sm: 'row' }} spacing={3} mt={1}>
                  ...
                  <Box flex={1}>
                    ...
                    <Typography variant="body2">
                      {formatUSD(destino.saldoActual)} +{' '}
                      {formatUSD(montoNum)} ={' '}
                    </Typography>
                    <Typography variant="h6" fontWeight={700} color="success.main">
                      {formatUSD(saldoDestinoPost!)}
                    </Typography>
                  </Box>
                </Stack>
                {origenOverdraft && (
