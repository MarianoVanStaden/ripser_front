...
export const MetricasPrioridadChart = ({ data }: MetricasPrioridadChartProps) => {
  ...
  const total = data.reduce((sum, item) => sum + item.totalLeads, 0);
        ...
        <Box sx={{ display: 'flex', justifyContent: 'center', my: 3 }}>
          <Box sx={{ position: 'relative', width: 200, height: 200 }}>
            <svg width="200" height="200" viewBox="0 0 200 200">
              {data.map((item, index) => {
                const percentage = (item.totalLeads / total) * 100;
                const previousPercentages = data
                  .slice(0, index)
                  .reduce((sum, prev) => sum + (prev.totalLeads / total) * 100, 0);
              ...
              <Box key={item.prioridad} sx={{ mb: 2 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', mb: 0.5 }}>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    {item.totalLeads} ({percentage.toFixed(1)}%)
                  </Typography>
                </Box>
                <Typography variant="caption" color="text.secondary" sx={{ ml: 3 }}>
                  Convertidos: {item.leadsConvertidos} • 
                  Tasa: {item.tasaConversion.toFixed(1)}% • 
                  Valor prom: ${item.promedioValorEstimado.toFixed(0)}
                </Typography>
              </Box>
            );
          })}
