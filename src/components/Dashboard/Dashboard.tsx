...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    documentoApi
      .getVentasEquiposPorVendedor(mesActual, mesActual)
      .then((rows) => {
        if (!cancelled) {
          setUnidadesRefrigeradasMes(rows.reduce((acc, r) => acc + r.totalNeto, 0));
        }
      })
      .catch(() => {
        if (!cancelled) setUnidadesRefrigeradasMes(null);
      });
    return () => {
      cancelled = true;
    };
