...
- [~] **FRONT-003** Mismo split para `NotasPedidoPage` (2.590), `AsistenciasPage` (2.528), `PresupuestosPage` (2.232), `DeliveriesPage` (2.219), `ComprasPedidosPage` (2.115), `TransferenciasPage` (2.064). **Criterio ajustado (2026-05-03):** mismo trade-off que FRONT-002 — target < 2.000 LOC + dialogs/tabs/helpers extraídos a una subcarpeta por página, en lugar de < 500 LOC estricto. *Avance (2026-05-03):*
    ...
    - *NotasPedidoPage: dos extracciones mecánicas. (Paso 1) Tipos (`TipoIva`, `TipoDescuento`, `ConvertFormData`) → [src/components/Ventas/NotasPedido/types.ts](src/components/Ventas/NotasPedido/types.ts). Constantes (`initialConvertForm`, `IVA_RATES`) → [src/components/Ventas/NotasPedido/constants.ts](src/components/Ventas/NotasPedido/constants.ts). (Paso 2) Helpers puros (`parseDeudaError`, `getTipoIvaLabel`) → [src/components/Ventas/NotasPedido/utils.ts](src/components/Ventas/NotasPedido/utils.ts). Iconos+labels de método de pago → [src/components/Ventas/NotasPedido/paymentMethodIcons.tsx](src/components/Ventas/NotasPedido/paymentMethodIcons.tsx) (NB: similar al de Facturacion pero **NO** consolidado — labels distintos en cada page, consolidar sería un cambio cosmético cross-module fuera de scope). 3 imports `@mui/icons-material` removidos del page (`MoneyIcon`, `CreditCardIcon`, `BankIcon`) ya que solo se usaban dentro de la función extraída. Resultado: `2.590 → 2.502 LOC` (-88, ~3.4%). Build, typecheck, lint, smoke verdes después de cada paso.*
    ...
    - *Pendiente en NotasPedidoPage (sesiones próximas): los 6 dialogs inline. Convert Dialog (~289 LOC, presupuesto → nota), View Dialog (~197 LOC), Edit Dialog (~144 LOC), Lead Conversion Dialog (~84 LOC), Billing Dialog (~116 LOC), Opciones Financiamiento Dialog (~77 LOC) — 907 LOC totales, 6 sesiones de extracción presentacional con Props interface explícita por dialog. Progresión esperada: orquestador queda en ~700-1.000 LOC.*
    - *Resto de FRONT-003 pendiente:* `AsistenciasPage`, `PresupuestosPage`, `DeliveriesPage`, `ComprasPedidosPage`, `TransferenciasPage`. **Orden recomendado de menor a mayor riesgo:** (1) `AsistenciasPage` (RRHH, no toca plata) — plantilla del patrón fuera de Ventas, (2) `PresupuestosPage`, (3) `DeliveriesPage`, (4) `TransferenciasPage`, (5) `ComprasPedidosPage`, (6) terminar `NotasPedidoPage`. Cada uno es 1 sesión + escribir/correr smoke específico siguiendo el patrón de [`e2e/modules/ventas/notas-pedido.smoke.test.ts`](e2e/modules/ventas/notas-pedido.smoke.test.ts).
...
- [x] **DOCS-003** Consolidar los 80+ `.md` sueltos del frontend. Borrar `FIX_*`, `DEBUG_*`, `URGENTE_*`, `SOLUCION_*`, `RESUMEN_*`, `IMPLEMENTACION_*`, `BACKEND_*` y similares (ya están en git history). Mantener en raíz solo `README.md`, `AUDITORIA_TECNICA.md`, `TECHNICAL_DEBT.md`, `DEPLOY_VPS.md`. Aceptación: ≤4 `.md` en raíz; índice [docs/README.md](docs/README.md) operativo. *Implementado (2026-05-03):*
    ...
    - *Por qué borrar en lugar de archivar: la fuente canónica de los fixes históricos es `git log` + cuerpo del commit; archivar a `docs/archive/` solo recrea ruido pasivo y reabre la puerta a "guardar todo por las dudas". El conocimiento durable por módulo ya estaba migrado a [docs/contextos/](docs/contextos/) antes de este refactor — los 100 archivos eran realmente derivables (FIX/DEBUG = git history) o frozen-in-time (análisis puntuales con timestamp implícito).*
    - *Out of scope, intencionalmente no tocado: `commands/*.md` (slash commands), `plans/*.md` (outputs de agentes), `playwright-report/*.md` (artefactos de test), `public/kpis/README.md` y `dist/kpis/README.md` (publicados con la app), `ripser/README.md` (scaffold huérfano que requiere decisión de housekeeping aparte: borrar la carpeta entera o no).*
- [x] **OPS-003** CI con build + tests + lint + typecheck + migraciones contra DB efímera. Aceptación: PR rojo si falla cualquiera de los 5. *Implementado en ambos repos: (1) [ripser_front/.github/workflows/ci.yml](https://github.com/) corre `lint` (ESLint), `typecheck` (tsc), `test` (Vitest+coverage), `build` (Vite). (2) [ripser_back/.github/workflows/ci.yml](https://github.com/) corre `unit-tests` (Surefire) en todas las ramas, `integration-tests` (Failsafe + Testcontainers MySQL, levanta migraciones Flyway contra DB limpia) en PRs a main, y `build` (mvn package). Concurrency cancel-in-progress configurado. **Pendiente del owner**: configurar Branch Protection Rules en GitHub UI marcando estos jobs como `Required` para que un PR rojo no pueda mergearse — eso es admin del repo, no código.*
- [~] **OPS-004** Probar plan de backup/restore de MySQL en staging. Documentar RPO/RTO. *Documentado: nuevo runbook en `ripser_back/docs/runbooks/backup-restore-mysql.md` con RPO=24h (objetivo ≤1h con binlog+S3) y RTO=2h. Cubre: política actual (mysqldump cron 03:00 ART, retención 14 días local), procedimientos detallados de backup manual y restore step-by-step, comandos de validación post-restore (counts, Flyway version, smoke checks vía Actuator), tabla de checks, limitaciones conocidas (no off-site, no PITR, no drill probado) y plan priorizado de mejoras (cron a S3 → drill cronometrado → binlogs + PITR). **Pendiente del owner**: ejecutar el drill cronometrado en staging para validar RTO real, y dar de alta el cron a S3 (sin el cual el VPS es SPOF físico).*
...
- 0 incidentes de schema drift en 90 días post `ddl-auto=validate`.
- p95 latencia HTTP visible en Prometheus.
## 10. Actualizaci�n de Refactor (Mayo 2026)

Durante la sesi�n actual se realizaron pruebas y extracciones sobre servicios cr�ticos (estrategia definida en BACK-004 y BACK-005):

- **Completado (BACK-004)**: Se logr� extraer DocumentoComercialMapper, DocumentoQueryService (read-only) y DocumentoCreditService, aligerando la clase principal DocumentoComercialServiceImpl conteniendo su complejidad.
- **Diferido (BACK-004b)**: La extracci�n de DocumentoIssuanceService (flujos de emisi�n y financiamiento propio como procesarFinanciamientoPropio, esolverMontoEntregaInicial, crearPrestamoDesdeFactura) ha sido pospuesta por considerarse en este momento un sub-flujo demasiado riesgoso (~250 LOC autocontenidas) y sin la cobertura total necesaria. Queda documentado como **next step**.
- **Diferido (BACK-005)**: Se pivot� para aplicar un patr�n similar a los otros 4 servicios target (EquipoFabricadoServiceImpl, ChequeServiceImpl, ReconciliacionStockServiceImpl y LeadMetricasServiceImpl), pero se decide posponer la implementaci�n hasta tener mayor holgura y redes de tests (los tests baseline est�n correctos y en verde, pero la extracci�n presenta riesgos similares a DocumentoIssuanceService).

Pr�ximos pasos: Avanzar en consolidaci�n de extracciones pendientes cuando exista mayor cobertura E2E o una red de test de integraci�n que soporte el redise�o transaccional de los flujos de "issuance" (emisi�n) y los otros cuatro servicios grandes.
>>>>>>> Stashed changes
