...
- [ ] **FRONT-003** Mismo split para `NotasPedidoPage` (2.590), `AsistenciasPage` (2.528), `PresupuestosPage` (2.232), `DeliveriesPage` (2.219), `ComprasPedidosPage` (2.115), `TransferenciasPage` (2.064). **Criterio ajustado (2026-05-03):** mismo trade-off que FRONT-002 — target < 2.000 LOC + dialogs/tabs/helpers extraídos a una subcarpeta por página, en lugar de < 500 LOC estricto. *Estado: ninguno tocado todavía. Total acumulado **13.748 LOC**.* **Bloqueante asumido: depende de FRONT-006.** Estos archivos tocan stock, transferencias, presupuestos→nota→factura, asistencias y compras a proveedores — partir handlers de negocio sin E2E que validen los golden paths es alto riesgo. Hacer FRONT-006 primero (suite Playwright) y después este split. **Orden recomendado de menor a mayor riesgo cuando se ejecute:** (1) `AsistenciasPage` (RRHH, no toca plata) — sirve de plantilla, (2) `PresupuestosPage`, (3) `DeliveriesPage`, (4) `TransferenciasPage`, (5) `ComprasPedidosPage`, (6) `NotasPedidoPage` (el más acoplado a facturación/stock, dejarlo último). Cada uno es 1 sesión.
