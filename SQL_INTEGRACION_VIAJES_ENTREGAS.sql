-- ============================================
-- INTEGRACIÓN COMPLETA: Entregas de Equipos + Viajes + Control de Entregas
-- ============================================

-- 1️⃣ TABLA: Detalle de entregas por viaje
-- Relaciona viajes con facturas y equipos específicos
...
-- 2️⃣ ACTUALIZAR: Tabla entregas_viaje (si necesita campos adicionales)
-- Agregar columnas sin especificar AFTER (por si no existen las columnas de referencia)
...
-- Primero agregar columnas conductor_id y vehiculo_id si no existen
ALTER TABLE entregas_viaje
...
SELECT 
    ...
    ev.vehiculo_id,
    COALESCE(v.patente, '') as vehiculo_patente,
    ev.total_equipos,
...
INNER JOIN detalle_documentos dd ON FIND_IN_SET(ef.id, dd.equipos_fabricados_ids) > 0
...
DROP PROCEDURE IF EXISTS sp_agregar_factura_a_viaje;
...
DROP PROCEDURE IF EXISTS sp_confirmar_entrega_equipo;
...
INSERT INTO entregas_viaje (fecha_viaje, conductor_id, vehiculo_id, observaciones)
VALUES ('2025-11-21', 1, 1, 'Viaje de prueba para integración');

-- Agregar la factura 171 al viaje (ejecutar el procedimiento)
CALL sp_agregar_factura_a_viaje(LAST_INSERT_ID(), 171, 1);
*/
...
SELECT 
    ...
    c.nombre as cliente_nombre
FROM entrega_viaje_detalle evd
INNER JOIN equipos_fabricados ef ON evd.equipo_fabricado_id = ef.id
INNER JOIN documentos_comerciales d ON evd.factura_id = d.id
INNER JOIN clientes c ON d.cliente_id = c.id
WHERE evd.viaje_id = 1
ORDER BY evd.orden_entrega;

-- Ver progreso de entregas por viaje
SELECT 
    viaje_id,
    estado_viaje,
    total_equipos,
    equipos_entregados,
    CONCAT(ROUND((equipos_entregados / total_equipos) * 100, 2), '%') as progreso
FROM vista_viajes_resumen
WHERE estado_viaje IN ('PLANIFICADO', 'EN_RUTA');
